...
| item | shipped | what |
|---|---|---|
| **Shadow model** (`fi_form.py`, `_shadow_score`, `tools/shadow_report.py`) | 2026-09-04 | Every predict tick scores each game a second time with a candidate model and records it in six ledger columns; the live pick, stake, alerts and cards are untouched. Fail-open, kill switch `NRFI_SHADOW_MODEL=disabled`, 13 tests pin the live path. Candidate loaded: the raw last-10 no-run fraction replaced by its empirical-Bayes shrunk version (K=65 starts), umpire input dropped. Its backtest is positive in all three splits but **within search noise on clean data (p=0.110)** -- which is why it runs as a shadow and not a ship. Nightly paired report to `data/diagnostics/shadow_report.json`, and a `/history` card (`ShadowModelCard`, `lib/shadow-compare.ts`) that builds the same comparison straight from Supabase, so it works while the GitHub cron is down. Found en route: rescheduled games carry the same game_pk on two dates with the original row graded under the wrong starters (T8.41). |

## ✅ Recently shipped (Aug 2026)

| item | shipped | what |
|---|---|---|
| **Pooled first-inning pitcher xwOBA + L2 0.50** (`fi_pitcher_pool.py`) | 2026-08-22 | The first model input that measures the first inning specifically: each starter's expected-wOBA allowed in first innings only, pooled over 2024-26 with shrinkage, strictly pre-game. First of ~95 candidates to clear the full protocol (three splits, search-aware null p=0.000, five shrinkage settings, independent leakage audit). Out of sample on 2026 the nightly No.1 goes .657 → .736 (+24.0u → +34.1u flat); August .538 → .769. Ships as 20-feature `lr_t1/lr_b1.json` + CIR calibrator refit on 2024-26 (backups `data/*.bak-2026-08-22-pre-fixwoba`), a committed running-sums state file (`data/fi_pitcher_pool.json`) advanced nightly from Savant and self-refreshed at predict time (fail-open), a `daily.yml` grade step, and a dashboard reason-map entry. **Reversal:** restore the three `.bak` files and remove `home_fi_xwoba` / `away_fi_xwoba` from `_T1/_B1_EXPECTED_FEATURES`. |
