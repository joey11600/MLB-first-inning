...
| 2026-05-03 | _pending_ | **T3.12** — Worst-day deep dive after 2-6 record on 8 STRONG bets (-4.55u, worst in 30d). Surfaced 4 structural findings (calibrator clamps P(NRFI) to [0.36, 0.66]; within-YRFI 0.37-0.40 "losing valley"; NRFI vs YRFI 11pp hit-rate gap; no slate-context). Added Variants G/H/I to A/B harness, then ran 2025 holdout test (`tools/test_variant_g_2025.py`) which collapsed Variant G's +6.27u in-sample lift to **+1.00u out-of-sample** (selection bias confirmed). Refined finding: the 0.37-0.40 band splits into a real losing 0.37-0.38 sub-band and a winning 0.38-0.40 sub-band. Added **Variant J** (skip ONLY 0.37-0.38). Reproduces on both samples: 30d 2026 (+5.19u) AND 2025 full season (+5.83u), combined 24 bets skipped, 7-17 (29% hit), -11.01u saved. Strongest variant signal to date but still below +10u-across-≥2-leak-free-folds ship bar. NOT shipped to production. Documented in `docs/KB.md`. |
