...
| 2 | [ ] | 3–4 hr | +0.5–1.5% edge | **Pitcher days-rest feature** (`days_since_last_start`). Trivial to add via `statsapi.player_stat_data`. Backtest before merging. |
| 3 | [ ] | 4–6 hr | +0.5–1.5% edge | **Wind-direction × park-orientation**. Currently have `wx_wind_kmh` (magnitude only). Wind blowing OUT to RF at Wrigley vs IN are opposite signals. |
