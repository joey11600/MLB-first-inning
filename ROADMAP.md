...
| 2026-05-03 | _pending_ | **T3.12** — Worst-day deep dive after 2-6 record on 8 STRONG bets (-4.55u, worst in 30d). Surfaced 4 structural findings (calibrator clamps P(NRFI) to [0.36, 0.66]; within-YRFI 0.37-0.40 "losing valley"; NRFI vs YRFI 11pp hit-rate gap; no slate-context). Added Variants G/H/I to A/B harness (`db/variants.py` + `tools/backfill_variants.py`). 30d backfill ranking: **VAR-G +6.27u vs PROD** (only winner; skips 0.37-0.40 YRFI band; tiny n=29 sample, selection bias risk); H/I/D/F/A/C/AC all regress. Documented in `docs/KB.md` new "Known structural limitations" section. NOT shipped to production until walk-forward gate is honest (currently blocked on per-game xera/whiff backfill from T3.11-AUDIT). |
